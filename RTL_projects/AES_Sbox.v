module AES_Sbox(rst,
	...
	//inputs
	input 		   rst;					//reset
	input  [127:0] sbox_in;			//input for sbox
	
	//outputs
	output [127:0] sbox_out;			//output from sbox

	//data types
	reg				[127:0] hello;
	reg  		 	[7:0]  state_in [3 :0][3:0];		//storing the input values as states
	reg				[7:0]  sub_box[15:0][15:0];
	reg  		 	[7:0]  sbox_t  [3 :0][3:0];			//substitute the values in state_in matrix from sub_box
	...
	assign sbox_out = hello;
	//This block of code converts the input state_in of bits directly into state_pt matrix 
	always @(*) begin
	...
	//Sbox instantiation
		if(rst) begin
			...
			hello <= {sbox_t[0][0],sbox_t[0][1],sbox_t[0][2],sbox_t[0][3],
						sbox_t[1][0],sbox_t[1][1],sbox_t[1][2],sbox_t[1][3],
						sbox_t[2][0],sbox_t[2][1],sbox_t[2][2],sbox_t[2][3],
						sbox_t[3][0],sbox_t[3][1],sbox_t[3][2],sbox_t[3][3]};

						
			$display("sbox_out=%h \t",sbox_out);
		end	
