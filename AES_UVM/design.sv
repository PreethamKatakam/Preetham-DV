...
module AES_cipher(rst,
	...
	reg  		 	[7:0]   mixcol_t  [3 :0][3:0];		//MixCol transformation matrix from shiftrow_t matrix	
...
//		$display("roundkey output=%h \t",op);
	...
	//Substitution box transformation task	
	task SubBytes( input [127:0] sbox_in,output reg [127:0] sbox_op);
	begin	
...
//		$display("sbox_in = %h",sbox_in);
		...
		for(i = 0; i<4 ; i = i+1) begin
			for(j = 0; j<4 ; j = j+1) begin
				sbox_t[i][j] = sub_box[state_in[i][j][7:4]][state_in[i][j][3:0]];
			end
		end			
	...
	//shift row transformation task
	task ShiftRows( input [127:0] ip, output [127:0] op);
	begin
	
//		$display("shift row ip = %h",ip);
		
		op[127:120] = ip[127:120];	op[95:88] = ip[95:88];		op[63:56] = ip[63:56];		op[31:24] = ip[31:24];
		op[119:112] = ip[87:80];	op[87:80] = ip[55:48];		op[55:48] = ip[23:16];		op[23:16] = ip[119:112];
		op[111:104] = ip[47:40];	op[79:72] = ip[15:8];		op[47:40] = ip[111:104];	op[15:8]  = ip[79:72];
		op[103:96]  = ip[7:0];		op[71:64] = ip[103:96];		op[39:32] = ip[71:64];		op[7:0]   = ip[39:32];
...
// 		$display("shiftrow output=%h \t",op);
	...
	//Mix Column transformation task
	task MixColumns( input [127:0] ip, output [127:0] op);
	begin
	
//		$display("Mix Col ip = %h",ip);
		...
		//MixCol transformation		
		for(i = 0; i<4 ; i = i+1) begin
			//if(i==0) begin
			for(j = 0; j<4 ; j = j+1) begin
				if(i==0) begin
					mixcol_t[i][j] = 
								(( state_in[0][j] << 1'b1) ^ ( {8{state_in[0][j][7]}} & 8'h1b )) ^
								(state_in[1][j] ^ (( state_in[1][j] << 1'b1) ^ ( {8{state_in[1][j][7]}} & 8'h1b ))) ^ 
								(state_in[2][j]) ^
								(state_in[3][j]);
				...
				if(i==1) begin
					mixcol_t[i][j] = 
								(state_in[0][j]) ^
								((state_in[1][j] << 1'b1) ^ ( {8{state_in[1][j][7]}} & 8'h1b )) ^ 
								(state_in[2][j] ^ (( state_in[2][j] << 1'b1) ^ ( {8{state_in[2][j][7]}} & 8'h1b ))) ^ 
								(state_in[3][j]);						
				end
				if(i==2) begin
					mixcol_t[i][j] = 
								(state_in[0][j]) ^ 
								(state_in[1][j]) ^ 
								(( state_in[2][j] << 1'b1) ^ ( {8{state_in[2][j][7]}} & 8'h1b )) ^ 
								(state_in[3][j] ^ (( state_in[3][j] << 1'b1) ^ ( {8{state_in[3][j][7]}} & 8'h1b )));	
				end
				if(i==3) begin
					mixcol_t[i][j] = 
								(state_in[0][j] ^ (( state_in[0][j] << 1'b1) ^ ( {8{state_in[0][j][7]}} & 8'h1b ))) ^ 
								(state_in[1][j]) ^ 
								(state_in[2][j]) ^ 
								(( state_in[3][j] << 1'b1) ^ ( {8{state_in[3][j][7]}} & 8'h1b ));	
				end
			end	
		...
		op = {mixcol_t[0][0],mixcol_t[1][0],mixcol_t[2][0],mixcol_t[3][0],
			mixcol_t[0][1],mixcol_t[1][1],mixcol_t[2][1],mixcol_t[3][1],
			mixcol_t[0][2],mixcol_t[1][2],mixcol_t[2][2],mixcol_t[3][2],
			mixcol_t[0][3],mixcol_t[1][3],mixcol_t[2][3],mixcol_t[3][3]};
		
//		$display("MixCol output=%h \t",op);
		disp_sMatrix(op);
...
//			$display(" keyExpansion_done =%h\t",keyExpansion_done);
			...
			//KeyExpansion(inti_key,round_key[1],round_key[2],round_key[3],round_key[4],round_key[5],round_key[6],round_key[7],round_key[8],round_key[9],round_key[10]);
		end
	end
	
	//Encryption rounds
  always @ (*) begin
//      $display(" %t keyExpansion_done =%h\t cipher_done=%d",$time,keyExpansion_done,cipher_done);
...
//			$display(" round =%h\t",round);
			...
			case(round)
				4'b0000: begin
							 AddRoundKey(plain_text,inti_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);
			
						 end
				4'b0001: begin
							 SubBytes(round_op,sbox_op);
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round1_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end
				4'b0010: begin
							 ...
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round2_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end
				4'b0011: begin
							 ...
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round3_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end		 
				4'b0100: begin
							 ...
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round4_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end		 
				4'b0101: begin
							 ...
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round5_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end		 
				4'b0110: begin
							 ...
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round6_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end		 
				4'b0111: begin
							 ...
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round7_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end		 
				4'b1000: begin
							 ...
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round8_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end		 
				4'b1001: begin
							 ...
							 ShiftRows(sbox_op,shift_op);
							 MixColumns(shift_op,mix_op);
							 AddRoundKey(mix_op,round9_key,round_op);
							 nxt_round = round+1;
//							 $display("round=%h \t",round);		
						 end		 
				4'b1010: begin
							 SubBytes(round_op,sbox_op);
							 ShiftRows(sbox_op,shift_op);
							 AddRoundKey(shift_op,round10_key,round_op);
				...
				/*default: begin
							for(i = 1; i<=9 ; i = i+1) begin
								if(i == round) begin
								 ...
								 ShiftRows(sbox_op,shift_op);
								 MixColumns(shift_op,mix_op);
								 AddRoundKey(mix_op,round_key[i],round_op);
								 nxt_round = round+1;
								 $display("round=%h \t",round);
								end	
							end	
